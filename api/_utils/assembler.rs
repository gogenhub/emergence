...
use data::{get_gene, get_gene_at, get_input, get_part, get_rules, has_input, Gene, Input};
use helpers::{get_group, make_plasmid_dna, make_plasmid_part, make_plasmid_title, map};
use serde::Serialize;
...
impl Assembler {
	...
	pub fn apply_rules(&self, gc: &mut GeneticCircuit) {
		...
		gc.genes.sort_by(|a, b| {
			let a_index = rules.gates.get(&get_group(&a.name)).unwrap();
			let b_index = rules.gates.get(&get_group(&b.name)).unwrap();
			a_index.cmp(b_index)
		});
	...
	pub fn make_dna(&self, genes: &Vec<Gene>) -> (String, String) {
		...
		for gene in genes {
			...
			for inp in &gene.inputs {
				...
				gates_dna += &part.seq;
				gates_plasmid += &make_plasmid_part(&part.kind, start, end, &part.name, promoter_colors.get(inp).unwrap_or(&"white".to_owned()));
			}

			let bio_gate = get_gene(&gene.name);
